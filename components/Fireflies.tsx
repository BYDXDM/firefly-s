...
export default function Fireflies() {
  ...
  useEffect(() => {
    const generated: Firefly[] = Array.from({ length: 35 }).map((_, i) => ({
      id: i,
      // 初始出生点
      top: `${Math.random() * 100}%`,
  ...
  return (
    <div className="fixed inset-0 w-full h-full pointer-events-none z-10 overflow-hidden mix-blend-screen">
      ...
      {/* 动画引擎 */}
      <style>{`
        /* 内层：纯粹的光芒呼吸闪烁（只动 opacity/transform，避免 box-shadow 每帧重绘） */
      ...
      {flies.map(fly => (
        ...
        >
          {/* 【内层元素】：负责自身发光、变大和透明度呼吸（光晕用静态阴影，不参与动画） */}
          <div
            ...
            style={{
              ...
              boxShadow: '0 0 10px 3px rgba(100, 255, 150, 0.5), 0 0 20px 6px rgba(50, 255, 100, 0.25)',

...
`/admin` 是文章发布后台，入口有管理员登录门：**无账号，仅需密钥**（环境变量 `ADMIN_PASSKEY`），登录后可发布文章/说说/杂谈、上传图片、更换站长头像（「站点设置」），以及在「内容管理」中对已发布文章进行**编辑与删除**。若设置了环境变量 `ADMIN_PASSKEY`，发布/上传/头像/建议管理接口均需携带该密钥；未设置时放行（仅建议本地使用）。登录尝试有频率限制（同 IP 每 5 分钟 10 次）。
